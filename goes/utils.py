...
def read_file_glm(abs_path, product='f', window=False):
    """
    ...
    product : str
        Product (out of events, groups, and flashes) to read. Only used for
        _read_file_glm_egf func. Default is 'f'
        Valid strings: 'f': Flashes, 'g': Groups, 'e': Events, 'a': All (events, groups, & flashes)
...
def _read_file_glm_fed(abs_path,  window=False):
    """
    Read an AWIPS-compliant GLM Flash Extent Density file into a glm_obj.
    Called by read_file_glm func
    ...
    data_dict['data_type'] = 'Flash Extent Density'

    glm_obj = LocalGLMFile(f_path, 'awips')
...
def _read_file_glm_egf(abs_path, product='f'):
    """
    Read a GLM file into a glm_obj
