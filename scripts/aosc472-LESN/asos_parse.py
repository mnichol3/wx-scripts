...
def get_asos_stations(asos_df):
    ...
    Returns
    -------
    station_dict : dictionary; key : str, value : tuple of float
        Dictionary where each key is a string representing the ASOS station
        names, and each key's value is a tuple of 2 floats, the first being the
        stations' longitude, the second being the station's latitude
    """
    station_dict = {}
    stations = asos_df.station.unique()

    for n in stations:
        temp_df = asos_df.loc[asos_df['station'] == n]
        station_dict[n] = (float(temp_df.iloc[0]['lon']), float(temp_df.iloc[0]['lat']))
